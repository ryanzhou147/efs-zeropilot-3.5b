...
int main() {
    ...
    uavcan_equipment_camera_gimbal_Mode _msg = sample_uavcan_equipment_camera_gimbal_Mode_msg();

    uint8_t buffer[UAVCAN_EQUIPMENT_CAMERA_GIMBAL_MODE_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_equipment_camera_gimbal_Mode_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_equipment_camera_gimbal_Mode decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = UAVCAN_EQUIPMENT_CAMERA_GIMBAL_MODE_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (uavcan_equipment_camera_gimbal_Mode_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_equipment_camera_gimbal_Mode_encode(&decoded_msg, buffer);
